...
# Analysis 
vertical_simulation_removal <- function(net,
                                        seed,
                                        num_sims,
                                        perc_remove,
                                        nk_landscape,
  ...
  df_complete_nr <- rugged_crawl_vertical(net = net,
                                          r_max = turns,
                                          sims = num_sims,
                                          landscape = nk_landscape, 
                                          p_exp = prob_exp, 
                                          age_thr = thr)
  ...
  df_complete_r <- rugged_crawl_vertical(net = net_rems,
                                          r_max = turns,
                                          sims = num_sims,
                                          landscape = nk_landscape, 
                                          p_exp = prob_exp, 
                                          age_thr = thr)
  df <- rbind(df_complete_nr,
              df_complete_r)
...
complete_df <- vertical_simulation_removal(net = net, 
                                           seed = 33, 
                                           num_sims = 2, 
                                           perc_remove = 0.1, 
...
all_results <- map_df(c(seq(from = 0, to = 0.2, by = 0.05)), 
                      vertical_simulation_removal, 
                      net = net,
                      seed = 77, 
                      num_sims = 25, 
                      perc_remove = 0.1, 
...
all_results <- all_results %>% 
  mutate(removal = rep(rep(c("no_removal", "removal"), each = 2500), 5), 
         p_exp = rep(c(seq(from = 0, to = 0.2, by = 0.05)), each = 5000))

srs <- all_results %>% 
  mutate(removal = as.factor(removal), 
         p_exp = as.factor(p_exp)) %>% 
  filter(time == 100) %>% 
  group_by(removal, p_exp) %>% 
...
srs %>% 
  ...
  geom_point(aes(p_exp, y = avg_learn, col = removal), pch = 4) +
  geom_pointrange(aes(x = p_exp, ymin = avg_learn_l, ymax = avg_learn_u, col = removal), linetype = "dotted")
