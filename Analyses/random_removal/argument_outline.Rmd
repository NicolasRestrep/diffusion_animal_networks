...
rdl %>% 
  ...
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2, color = "#A6761D") + 
  scale_color_manual(values = c("#1B9E77", "#D95F02", "#7570B3", "#A6761D")) +
  geom_line(alpha = 0.7) +
  facet_wrap(~net) + 
...
We can also examine the initial average efficiency across the different networks: 

```{r}
# Inverse efficiency function 
...
# Get efficiencies 
effs <- map_dbl(all_networks, inverse_efficiency)

# Create dataframe 
eff_df <- tibble(
  species = c(rep("elephant", 3), 
              rep("dolphin", 6), 
              rep("baboon", 6)), 
  wave = c(1:3,1:6, 1:6), 
  efficiency = effs
...
summ_eff <- eff_df %>% 
  ...
  summarise(`median` = median(efficiency), 
            `standard deviation` = sd(efficiency))

kableExtra::kable(summ_eff, format = "html", caption = "Efficiency Summaries") %>% 
